<x-layouts.app title="Hasil Kuis - Mochin-Kuis">
    <div class="max-w-2xl mx-auto">
        {{-- Score Summary --}}
        <div class="bg-white rounded-2xl border border-gray-200 p-8 text-center mb-8 shadow-sm relative overflow-hidden">
            ...
            <div class="flex items-center justify-center gap-2 mb-1">
                ...
                <h1 class="text-lg font-bold text-gray-900">{{ $attempt->quiz->title }}</h1>
            </div>
            <p class="text-sm text-gray-500 mb-6">Hasil pengerjaan kuis</p>
            ...
            <div class="inline-flex items-center justify-center w-32 h-32 rounded-full border-4 mb-4
                ...
                <div>
                    <div class="text-4xl font-bold {{ $attempt->scorePercentage() >= 70 ? 'text-green-600' : ($attempt->scorePercentage() >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
            ...
            <div class="grid grid-cols-3 gap-4 mt-6">
                ...
                <div class="bg-gray-50 rounded-xl p-3">
                    <p class="text-2xl font-bold text-red-600">{{ $attempt->total_questions - $attempt->score }}</p>
                    <p class="text-xs text-gray-500">Salah</p>
                </div>
                <div class="bg-gray-50 rounded-xl p-3">
                    <p class="text-2xl font-bold text-blue-900">{{ $attempt->total_questions }}</p>
        ...
        <div class="space-y-3">
            @foreach($attempt->answers->sortBy('question.order') as $index => $answer)
                <div class="bg-white rounded-2xl border-2 {{ $answer->is_correct ? 'border-green-200' : 'border-red-200' }} p-5 shadow-sm">
                    <div class="flex items-start gap-3">
                        <span class="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold {{ $answer->is_correct ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between gap-2">
                                <p class="font-medium text-gray-900 mb-2 leading-relaxed">
